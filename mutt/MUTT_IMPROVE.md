...
2.  **Dynamic Sidebar**
    ...
    *   **Action:** Add macro: `macro index,pager B '<enter-command>toggle sidebar_visible<enter>'`. [x]
...
2.  **Global Thread Manipulation**
    ...
    *   **Action:** Bind `<Esc>v` (or similar) to `collapse-all`. [x]
...
1.  **Address Book Integration (`khard`)**
    ...
    *   **Action:** Install `khard` and set `query_command = "khard email --parsable '%s'"` in `muttrc`. [x]

2.  **Fuzzy Search (`fzf`)**
    *   **Improvement:** Integrate `fzf` for fuzzy searching mailboxes or selecting contacts.
    *   **Action:** Create a macro that pipes the mailbox list or alias list to `fzf` and acts on the selection (change folder or insert address). [x]
...
3.  **Sending Backend (`msmtp`)**
    ...
    *   **Action:** Configure `~/.msmtprc` and set `set sendmail = "/usr/bin/msmtp"` in `muttrc`. [x]
...
4.  **GPG Encryption/Signing**
    ...
    *   **Action:** Populate `mutt/crypto/gpg.rc` with GPG command definitions and source it in `muttrc`. [x]

5.  **Desktop Notifications**
    *   **Current:** Visual beep/sound.
    *   **Improvement:** Trigger system notifications (e.g., `notify-send`) when new important mail arrives.
    *   **Action:** Set `new_mail_command` to a script that filters for priority mail and sends a desktop notification. [x]
...
6.  **Spam Handling Macro**
    ...
    *   **Action:** `macro index S "<save-message>=[Gmail]/Spam<enter>"` (Account dependent). [x]
