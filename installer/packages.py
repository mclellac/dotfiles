    ...
    try:
        if package_manager == "dnf":
            result = subprocess.run(
                ["dnf", "list", "installed", package], capture_output=True, text=True
            )
            ...
            result = subprocess.run([pip_cmd, "show", package], capture_output=True, text=True)
        ...
        console.print(
            "[bold red]Error:[/bold red] {} not found. Please make sure it is installed.".format(
                package_manager
            )
        )
        return False
...
def install_packages(packages: list, package_manager: str) -> None:
    ...
    try:
        ...
        if not_installed:
            ...
            console.print(
                f"[bold green]All {package_manager} packages have already been installed.[/bold green]"
            )
    except subprocess.CalledProcessError as e:
        console.print(
            f"[bold red]Error:[/bold red] Failed to install packages with {package_manager}: {e}"
        )
...
def is_copr_repo_enabled(repo_name: str) -> bool:
    ...
    try:
        result = subprocess.run(
            ["sudo", "dnf", "repolist"], capture_output=True, text=True, check=True
        )
        ...
        console.print(
            f"[bold red]Error:[/bold red] Failed to check if COPR repository {repo_name} is enabled: {e}"
        )
        return False
    except FileNotFoundError:
        console.print(
            "[bold red]Error:[/bold red] dnf not found. Please make sure it is installed."
        )
        return False
